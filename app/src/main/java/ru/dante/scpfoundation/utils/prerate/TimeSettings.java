...
class TimeSettings
{
    private static final long FIRST_SHOW_INTERVALE=7*24*60*60*1000;// Через 7 дней показывать первый диалог
    private static final long SHOW_LATER_INTERVALE=3*24*60*60*1000;// Через 3 дней показывать если нажали показать позже
    ...
    static boolean needShowPreRateDialog(Context cntx)
    {
        ...
        {
            long lastShowTime=getLastShowTime(cntx);
