...
class TimeSettings
{
    private static final long FIRST_SHOW_INTERVALE = 14 * 24 * 60 * 60 * 1000;// Через 7 дней показывать первый диалог
    ...
    {
        return cntx.getSharedPreferences(cntx.getPackageName()+"app install", Context.MODE_PRIVATE);
    }
}
